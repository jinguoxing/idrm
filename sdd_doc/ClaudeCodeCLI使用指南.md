...
本指南中所有涉及的规范文档路径（如 `sdd_doc/spec/`）都是基于**IDRM项目的实际目录结构**。
...
```bash
# IDRM项目示例
claude --files "sdd_doc/spec/constitution.md" "..."
...
**建议**：
1. 确认您项目中规范文档的实际位置
2. 将本指南中的所有 `sdd_doc/spec/` 替换为您的实际路径
...
# 然后输入提示词
> 读取sdd_doc/spec/constitution.md，理解项目规范
...
```bash
# 一次性执行单个命令
claude "读取sdd_doc/spec/constitution.md，总结项目规范"

# 带文件上下文
claude --files "sdd_doc/spec/**/*.md" "实现新功能"
```
...
然后输入：
```
读取 sdd_doc/spec/constitution.md
...
```bash
claude --files "sdd_doc/spec/architecture/layered-architecture.md" \
       --files "sdd_doc/spec/architecture/dual-orm-pattern.md" \
  "基于前面的规范，创建技术实施方案"
```
...
```bash
# 实施Task 1
claude --files "sdd_doc/spec/coding-standards/go-style-guide.md" \
       --files "sdd_doc/spec/coding-standards/naming-conventions.md" \
...
> 读取 sdd_doc/spec/architecture/layered-architecture.md
> 读取 sdd_doc/spec/architecture/dual-orm-pattern.md
...
> Phase 4: 实施Task 1
> 读取 sdd_doc/spec/coding-standards/go-style-guide.md
...
```bash
claude --files "sdd_doc/spec/coding-standards/code-review-checklist.md" \
...
```bash
# 只读取需要的文档
claude --files "sdd_doc/spec/architecture/dual-orm-pattern.md" \
  "实现新的model"

# 读取多个相关文档
claude --files "sdd_doc/spec/architecture/*.md" \
       --files "model/resource_catalog/category/*.go" \
...
aliases:
  review: "--files 'sdd_doc/spec/coding-standards/code-review-checklist.md'"
  spec: "--files 'sdd_doc/spec/**/*.md'"
```
...
case $1 in
  "read")
    claude --files "sdd_doc/spec/**/*.md" "总结项目规范"
    ;;
  "check")
    claude --files "sdd_doc/spec/coding-standards/code-review-checklist.md" \
           --files "$2" \
...
- name: Code Review
  run: |
    claude --files "sdd_doc/spec/coding-standards/*.md" \
           --files "**/*.go" \
      "生成代码质量报告" > report.md
```
...
- `sdd_doc/spec/constitution.md`
- `sdd_doc/spec/architecture/`
- `sdd_doc/spec/coding-standards/`
