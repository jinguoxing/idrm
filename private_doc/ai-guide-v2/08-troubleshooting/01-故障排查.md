# 故障排查指南

> Troubleshooting - 遇到问题不要慌

---

## 🎯 本文档目标

快速诊断和解决AI编程过程中的常见问题。

---

## 📋 问题分类

### 1. 工具配置问题
### 2. AI生成代码问题
### 3. 编译和运行问题
### 4. 测试问题
### 5. 质量门禁问题
### 6. Git和协作问题

---

## 🔧 1. 工具配置问题

### Problem 1.1: Cursor不读取.cursorrules

**症状**：
- Cursor不按规范生成代码
- AI不理解项目规范

**诊断**：
```bash
# 检查.cursorrules是否存在
ls -la .cursorrules

# 检查文件内容
cat .cursorrules
```

**解决方案**：
```bash
# 方案1：重启Cursor
# Cmd+Q 完全退出
# 重新打开项目

# 方案2：检查文件位置
# .cursorrules必须在项目根目录

# 方案3：重新创建
cp private_doc/spec/tools/cursor/.cursorrules.template .cursorrules
```

**预防措施**：
- 定期检查.cursorrules是否存在
- 将.cursorrules加入版本控制

---

### Problem 1.2: Claude CLI连接失败

**症状**：
```
Error: API key not valid
或
Error: Connection timeout
```

**诊断**：
```bash
# 检查API Key
echo $ANTHROPIC_API_KEY

# 测试连接
claude "hello"
```

**解决方案**：
```bash
# 方案1：设置API Key
export ANTHROPIC_API_KEY="your-key-here"

# 方案2：写入配置文件
echo 'export ANTHROPIC_API_KEY="your-key"' >> ~/.zshrc
source ~/.zshrc

# 方案3：检查网络
ping api.anthropic.com
```

---

### Problem 1.3: Kiro.dev Steering不生效

**症状**：
- Kiro生成的代码不符合IDRM规范
- 没有引用项目规范

**诊断**：
```bash
# 检查steering文件
ls -la .kiro/steering/

# 检查内容
cat .kiro/steering/idrm-rules.md
```

**解决方案**：
1. 确保文件路径正确：`.kiro/steering/*.md`
2. 在Kiro界面中手动刷新Steering
3. 重新连接项目

---

## 💻 2. AI生成代码问题

### Problem 2.1: AI生成代码不符合规范

**症状**：
- 函数>50行
- 没有中文注释
- 不遵循分层架构

**诊断**：
检查prompt是否引用了规范：
```
❌ 错误：
"创建Category Handler"

✅ 正确：
@spec/architecture/layered-architecture.md
@spec/coding-standards/go-style-guide.md
创建Category Handler
要求：
- 函数<50行
- 中文注释
- 遵循分层架构
```

**解决方案**：
**方案1：完善Prompt**
```
@private_doc/spec/core/workflow.md
@private_doc/spec/architecture/layered-architecture.md
@private_doc/spec/coding-standards/go-style-guide.md

{你的需求}

要求：
1. 遵循5阶段工作流
2. 分层架构：Handler → Logic → Model
3. 函数<50行
4. 完整中文注释
5. 错误处理用%w
```

**方案2：迭代修正**
```
@刚生成的代码
@spec/coding-standards/go-style-guide.md

修复这个代码：
1. 函数>50行的拆分
2. 补充中文注释
3. 优化错误处理
```

---

### Problem 2.2: AI幻觉问题

**症状**：
- 生成不存在的函数/包
- 引用错误的API
- 逻辑明显错误

**示例**：
```go
// AI幻觉示例
import "github.com/notexist/package"  // ❌ 包不存在

model.FindAll() // ❌ 方法不存在
```

**诊断**：
```bash
# 编译检查会发现
go build ./...
# undefined: xxx
```

**解决方案**：
**方案1：明确引用现有代码**
```
@model/resource_catalog/category/interface.go

参考这个接口，实现Logic层
不要自己编造方法名
```

**方案2：分步验证**
```
1. 先让AI列出需要调用的方法
2. 验证方法是否存在
3. 再让AI生成代码
```

**方案3：人工Review**
- 编译检查
- 仔细阅读生成代码
- 可疑的地方立即验证

---

### Problem 2.3: AI生成代码过于复杂

**症状**：
- 过度设计
- 引入不必要的抽象
- 代码难以理解

**解决方案**：
```
@生成的代码

简化这个实现：
1. 移除不必要的抽象
2. 使用更直接的方式
3. 保持简单（KISS原则）
4. 参考现有代码风格
```

---

## 🔨 3. 编译和运行问题

### Problem 3.1: go build失败

**症状**：
```
undefined: xxx
cannot use xxx as type yyy
```

**诊断步骤**：
```bash
# 1. 清理缓存
go clean -cache

# 2. 更新依赖
go mod tidy

# 3. 重新编译
go build ./...
```

**常见原因**：
1. **Import路径错误**
```go
// ❌ 错误
import "idrm/model/category"

// ✅ 正确
import "devops.aishu.cn/AISHUDevOps/AnyFabric/_git/idrm/model/category"
```

2. **循环依赖**
```
Handler → Logic → Handler  // ❌
```
解决：检查import，确保单向依赖

3. **类型不匹配**
```go
// 检查函数签名
// 检查接口实现
```

---

### Problem 3.2: 运行时panic

**症状**：
```
panic: runtime error: invalid memory address
panic: index out of range
```

**诊断**：
```bash
# 查看stack trace
# panic信息会显示出错位置
```

**常见原因**：
1. **空指针**
```go
// ❌ 未检查nil
result := model.FindOne(id)
name := result.Name  // panic if result is nil

// ✅ 检查nil
result, err := model.FindOne(id)
if err != nil || result == nil {
    return err
}
name := result.Name
```

2. **数组越界**
```go
// ❌ 未检查长度
item := items[0]  // panic if items is empty

// ✅ 检查长度
if len(items) > 0 {
    item := items[0]
}
```

---

## 🧪 4. 测试问题

### Problem 4.1: 测试覆盖率不足

**症状**：
```
coverage: 65.5% of statements
```

**诊断**：
```bash
# 生成覆盖率报告
go test -coverprofile=coverage.out ./...
go tool cover -html=coverage.out -o coverage.html

# 打开HTML查看未覆盖代码
open coverage.html
```

**解决方案**：
1. **补充边界测试**
```go
tests := []struct {
    name string
    input int64
    want error
}{
    {"正常值", 1, nil},
    {"零值", 0, ErrInvalid},      // ← 补充
    {"负值", -1, ErrInvalid},      // ← 补充
    {"最大值", math.MaxInt64, nil}, // ← 补充
}
```

2. **补充错误路径**
```go
// 测试错误情况
mockModel.EXPECT().
    FindOne(gomock.Any(), gomock.Any()).
    Return(nil, errors.New("db error"))  // ← 模拟错误
```

3. **用AI生成**
```
@logic/createcategorylogic.go
@coverage.html

为未覆盖的代码生成测试用例
```

---

### Problem 4.2: Mock不工作

**症状**：
```
missing call(s) to *MockModel.Insert(...)
unexpected call to *MockModel.FindOne(...)
```

**诊断**：
Mock expectation和实际调用不匹配

**解决方案**：
```go
// ❌ 错误：参数不匹配
mockModel.EXPECT().
    Insert(gomock.Any(), &Category{Name: "test"}).  // 期望特定对象
    Return(nil)

// 实际调用可能是不同的对象实例

// ✅ 正确：使用gomock.Any()
mockModel.EXPECT().
    Insert(gomock.Any(), gomock.Any()).  // 接受任何参数
    Return(nil)

// 或使用Matcher
mockModel.EXPECT().
    Insert(gomock.Any(), 
        gomock.Cond(func(x interface{}) bool {
            cat := x.(*Category)
            return cat.Name == "test"
        })).
    Return(nil)
```

---

## 🚪 5. 质量门禁问题

### Problem 5.1: golangci-lint报错太多

**症状**：
```
hundreds of lint errors
```

**优先级处理**：
```bash
# 1. 只看error级别
golangci-lint run --max-issues-per-linter=10

# 2. 只看特定linter
golangci-lint run --disable-all --enable=errcheck

# 3. 自动修复
golangci-lint run --fix
```

**常见错误**：
1. **errcheck**: 未检查错误
```go
// ❌
file.Close()

// ✅
defer file.Close()
// 或
if err := file.Close(); err != nil {
    log.Error(err)
}
```

2. **ineffassign**: 无效赋值
```go
// ❌
result := doSomething()
result = doOtherthing()  // 第一次赋值被覆盖

// ✅
result := doOtherthing()
```

---

### Problem 5.2: 测试超时

**症状**：
```
panic: test timed out after 10m
```

**诊断**：
```bash
# 查看哪个测试慢
go test -v ./... | grep PASS

# 带超时运行
go test -timeout 30s ./...
```

**解决方案**：
1. **优化测试**
- 使用内存数据库
- Mock外部调用
- 并行测试

2. **增加超时**
```bash
go test -timeout 5m ./...
```

---

## 📦 6. Git和协作问题

### Problem 6.1: 合并冲突

**症状**：
```
CONFLICT (content): Merge conflict in xxx.go
```

**解决步骤**：
```bash
# 1. 查看冲突文件
git status

# 2. 手动解决冲突
# 编辑文件，移除冲突标记
<<<<<<< HEAD
你的代码
=======
他人的代码
>>>>>>> branch-name

# 3. 标记已解决
git add xxx.go

# 4. 继续合并
git commit
```

**用AI辅助**：
```
这两段代码有冲突：

<<<<<<< HEAD
{你的代码}
=======
{他人的代码}
>>>>>>> branch

请帮我合并，保留两者的功能
```

---

### Problem 6.2: PR被拒绝

**常见原因**：
1. **不符合规范**
- 检查code-review-checklist.md
- 用AI Review自己的代码

2. **测试不通过**
- 本地运行所有测试
- 修复失败的测试

3. **缺少文档**
- 补充PR描述
- 更新相关文档

---

## 🆘 求助流程

### 1. 自己尝试解决（15分钟）
- 查本文档
- 查FAQ
- 查Google/StackOverflow

### 2. AI辅助诊断（10分钟）
```
@错误信息
@相关代码

问题：{描述问题}
已尝试：{尝试的方案}

请帮我诊断和解决
```

### 3. 团队求助
- Slack/钉钉求助
- 附带：
  - 问题描述
  - 错误信息
  - 已尝试方案
  - 相关代码

### 4. 创建Issue
如果是工具/框架bug，创建Issue：
- 最小可复现示例
- 环境信息
- 详细描述

---

## 📋 快速诊断清单

遇到问题时，按顺序检查：

### 编译问题
- [ ] go mod tidy
- [ ] go clean -cache
- [ ] 检查import路径
- [ ] 检查循环依赖

### 运行问题
- [ ] 检查配置文件
- [ ] 检查数据库连接
- [ ] 检查端口占用
- [ ] 查看日志

### 测试问题
- [ ] 清理测试数据
- [ ] 检查Mock设置
- [ ] 隔离测试环境
- [ ] 查看覆盖率报告

### AI工具问题
- [ ] 检查配置文件
- [ ] 重启工具
- [ ] 完善Prompt
- [ ] 引用规范

---

**遇到问题别慌，照着排查！** 🔍
