# 场景1：新增功能模块

> 完整功能开发（Kiro.dev为主）

---

## 📋 场景

**需求**：添加"资源分类"管理功能  
**复杂度**：中等（~500行）  
**工具**：Kiro.dev + Cursor  
**时间**：2-3小时

---

## 🔄 Phase 0: Context（10分钟）

### 在Kiro或Cursor中阅读规范

```
阅读以下规范：
- private_doc/spec/core/project-charter.md
- private_doc/spec/architecture/layered-architecture.md
- private_doc/spec/architecture/dual-orm-pattern.md
- private_doc/spec/coding-standards/go-style-guide.md
```

---

## 📝 Phase 1: Specify（20分钟）

### 使用Kiro.dev

1. **创建新Spec**
```
Kiro面板 → New Spec
名称：Resource Category Management
```

2. **描述功能**
```
我要添加资源分类管理功能

功能包括：
- 创建分类（POST /api/v1/categories）
- 查询分类列表（GET /api/v1/categories）
- 查询分类详情（GET /api/v1/categories/:id）
- 更新分类（PUT /api/v1/categories/:id）
- 删除分类（DELETE /api/v1/categories/:id，软删除）
- 支持父子关系（最多3层）

请生成Requirements，使用EARS notation
```

3. **Kiro生成requirements.md**

示例：
```markdown
## User Stories

AS a 数据管理员
I WANT 管理资源分类
SO THAT 我可以组织和分类数据资源

## Acceptance Criteria (EARS)

WHEN 用户创建分类时提交有效数据
THE SYSTEM SHALL 返回201和创建的分类ID

WHEN 用户创建分类时提交无效数据
THE SYSTEM SHALL 返回400和验证错误信息

WHEN 用户查询分类列表
THE SYSTEM SHALL 返回分页的分类列表

## Technical Constraints
- MUST follow layered architecture
- MUST implement dual ORM
- Functions MUST be < 50 lines
```

**质量检查 - Gate 1**：
- [ ] 用户故事完整
- [ ] EARS notation
- [ ] 技术约束明确

---

## 🎨 Phase 2: Design（30分钟）

### Kiro自动生成design.md

包含：

**1. 文件结构**
```
model/resource_catalog/category/
├── interface.go
├── types.go
├── factory.go
├── gorm_dao.go
└── sqlx_model.go

api/internal/logic/resource_catalog/category/
├── createcategorylogic.go
├── listcategorylogic.go
├── getcategorylogic.go
├── updatecategorylogic.go
└── deletecategorylogic.go

api/internal/handler/resource_catalog/category/
├── createcategoryhandler.go
├── listcategoryhandler.go
├── getcategoryhandler.go
├── updatecategoryhandler.go
└── deletecategoryhandler.go
```

**2. 接口定义**
```go
type Model interface {
    Insert(ctx context.Context, data *Category) error
    FindOne(ctx context.Context, id int64) (*Category, error)
    List(ctx context.Context, page, size int) ([]*Category, int64, error)
    Update(ctx context.Context, data *Category) error
    SoftDelete(ctx context.Context, id int64) error
}
```

**3. 数据结构**
```go
type Category struct {
    ID        int64     `gorm:"column:id;primaryKey" db:"id"`
    Name      string    `gorm:"column:name" db:"name"`
    Code      string    `gorm:"column:code" db:"code"`
    ParentID  *int64    `gorm:"column:parent_id" db:"parent_id"`
    Level     int       `gorm:"column:level" db:"level"`
    CreatedAt time.Time `gorm:"column:created_at" db:"created_at"`
    UpdatedAt time.Time `gorm:"column:updated_at" db:"updated_at"`
    DeletedAt *time.Time `gorm:"column:deleted_at" db:"deleted_at"`
}
```

**质量检查 - Gate 2**：
- [ ] 符合分层架构
- [ ] 文件清单完整
- [ ] 接口定义清晰

---

## 📋 Phase 3: Tasks（20分钟）

### Kiro生成tasks.md

```markdown
## Task 1: 创建Model接口和类型定义
Status: ⏸️ Not Started
Files:
- model/resource_catalog/category/interface.go
- model/resource_catalog/category/types.go
Estimated Lines: 80

## Task 2: 实现GORM DAO
Status: ⏸️ Not Started
Depends on: Task 1
Files:
- model/resource_catalog/category/gorm_dao.go
Estimated Lines: 120

## Task 3: 实现SQLx Model
Status: ⏸️ Not Started
Depends on: Task 1
Files:
- model/resource_catalog/category/sqlx_model.go
Estimated Lines: 100

## Task 4: 实现Factory
Status: ⏸️ Not Started
Depends on: Task 2, Task 3
Files:
- model/resource_catalog/category/factory.go
Estimated Lines: 40

## Task 5: 创建分类Logic
Status: ⏸️ Not Started
Depends on: Task 4
Files:
- api/internal/logic/resource_catalog/category/createcategorylogic.go
Estimated Lines: 45

## Task 6-9: 其他CRUD Logic...
## Task 10-14: 所有Handler...
```

**质量检查 - Gate 3**：
- [ ] 每个task <50行
- [ ] 依赖关系清晰

---

## 💻 Phase 4: Implement（1-2小时）

### 使用Kiro点击Task执行

或者切换到Cursor快速实施：

```
@private_doc/spec/coding-standards/go-style-guide.md
@tasks.md

执行Task 1：创建Model接口和类型

要求：
- 完整中文注释
- 遵循编码规范
```

### 逐个完成Tasks

每完成一个：
1. Review代码
2. 运行测试
3. 更新task状态为✅

**质量检查 - Gate 4**：
```bash
go build ./...
go test -cover ./...
golangci-lint run
```

---

## ✅ 完成标准

- [ ] 所有tasks完成
- [ ] 编译通过
- [ ] 测试覆盖>80%
- [ ] Lint无错误
- [ ] Code Review通过
- [ ] 文档已更新

---

**完整功能，严格流程！** 🎯
